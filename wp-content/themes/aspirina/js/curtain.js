...
(function($, window, document, undefined) {
  ...
  Plugin.prototype = {
    init: function() {
      ...
      if (this.options.mobile) {
        this.scrollEl = this.$element;
		  ...
		  this.scrollEl = $(body,html,document);
	  }
      ...
      var callbackImageLoaded = function() {
        // console.log('callback imagesLoaded');
        self.setDimensions();
    ...
    scrollToPosition: function(direction) {
      ...
      if (direction === 'up' || direction == 'down') {
        ...
        var index = $("#" + direction).index(),
          speed = Math.round(Math.abs(self.currentIndex - index) * (this.options.scrollSpeed * 4) / self.$liLength);
        ...
        if (self.webkit) {
          ...
          if (self.hasParallax) {
            ...
            for (var i = 0, n = self.$parallax.length; i < n; i++) {
              ...
              $(self.$parallax.get(i)).css({
                paddingTop: -(((docTop - self.currentP) * -(parallaxSpeed)))
              });
            }
          }
        }
    ...
    setEvents: function() {
      ...
      if (self.options.curtainLinks) {
        $(self.options.curtainLinks).on('click', function(e) {
          ...
          if (!self.isHashIsOnList(target.substring(1)) && position) {
            return false;
          }
          var position = self.$elDatas[$(target).index()]['data-position'] || null;
    ...
    setHash: function(hash) {
      var urlToTrack = window.location.pathname + '#' + hash;
      // "HARD FIX"
      el = $('[href=#' + hash + ']');
	  ...
	  __gaTracker('send', 'pageview', urlToTrack);
    },
    setCache: function() {
      var self = this;
